...
with col_right:
    ...
    with tab_dash:
        ...
        <div style="display: flex; justify-content: center; align-items: center; margin-top: 10px; margin-bottom: 10px;">
          <div style="position: relative; width: 140px; height: 140px;">
            <svg class="transform" style="transform: rotate(-90deg); width: 140px; height: 140px;" viewBox="0 0 160 160">
              <circle cx="80" cy="80" r="{radius}" fill="none" stroke="#F1F5F9" stroke-width="12" />
              {svg_circles}
            </svg>
            <div style="position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center;">
              <span style="font-size: 8px; font-weight: bold; text-transform: uppercase; color: #64748B;">Total CO2e</span>
              <span style="font-size: 16px; font-weight: 800; color: {text_color};">{projected_tons:.2f}t</span>
            </div>
          </div>
        </div>
        """
    ...
    with tab_ledger:
        ...
        if len(hist) >= 2:
            ...
            sparkline_html = f"""
            <div style="background: {bg_color}; border: 1px solid {card_border}; border-radius: 4px; padding: 4px; margin-top: 5px; margin-bottom: 10px;">
              <svg viewBox="0 0 500 80" style="width: 100%; height: 80px;" preserveAspectRatio="none">
                <line x1="10" y1="{target_y}" x2="490" y2="{target_y}" stroke="#EF4444" stroke-width="1" stroke-dasharray="3 3" opacity="0.6" />
                <polyline fill="none" stroke="#059669" stroke-width="2.5" points="{points_str}" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </div>
            """
        ...
        if len(hist) == 0:
            ...
            for entry in hist:
                ...
                st.markdown(
                    f"""
                    <div style="border: 1px solid {card_border}; border-radius: 4px; padding: 10px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; background: {bg_color};">
                      <div>
                        <span style="font-size: 11px; font-weight: bold; color: {text_color};">{formatted_date}</span><br/>
                        <span style="font-size: 9px; color: #64748B;">
                          Trans: {entry['transport']/1000.0:.1f}t | 
                          Energy: {entry['energy']/1000.0:.1f}t | 
                          Diet: {entry['diet']/1000.0:.1f}t | 
                          Waste: {entry['waste']/1000.0:.1f}t
                        </span>
                      </div>
                      <div style="display: flex; align-items: center; gap: 15px;">
                        <span style="font-size: 11px; font-weight: 800; color: #059669;">{entry['total']/1000.0:.2f} t</span>
                      </div>
                    </div>
                    """,
                    unsafe_allow_html=True
